...
import {countUrbanRural,countUrbanRuralMagyar} from "../DataQueries/countUrbanRural";
...
class PieChartOfUrbanRural extends HTMLElement {
    ...
    render() {
        ...
        innerHeader.innerText = `A Maros megyében vizsgázott magyar tanulók környezeti eloszlása:`;
        ...
        for (let i = 2015; i <= 2020; i++) {
            ...
            containerChartMagyar.classList.add(`piechartmagyar${i}`);
            ...
            countUrbanRuralMagyar(i).then(data=>{
                const div = d3.select(`.piechartmagyar${i}`);
                const width = window.innerWidth / 5;
                const height = window.innerHeight / 5;
                const radius = Math.min(width, height) / 2;
                const colorScale = d3.scaleOrdinal(["#E53935", "#D81B60", "#8E24AA", "#5E35B1", "#3949AB", "#1E88E5", "#039BE5", "#00ACC1", "#00897B", "#43A047", "#7CB342", "#C0CA33", "#FDD835", "#FFB300", "#FB8C00", "#F4511E", "#6D4C41", "#757575", "#546E7A"]);
